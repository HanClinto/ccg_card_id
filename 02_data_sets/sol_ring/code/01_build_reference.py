...
BULK_DATA_PATH = os.path.join(SCRYFALL_CACHE_DIR, "all_cards.json")
...
hash_methods = ["phash", "ahash", "dhash", "crhash", "whash", "whash_db4"] #,  #["brief"] #, "phash"]
...
def hash_file_imagehash(path, method, hash_size):
            ...
            for root, dirs, files in os.walk(DATASET_TEST_DIR):
                for file in files:
                    ...
                    if dists:
                        ...
                        print(f" Correct ID for {short_filename} is {correct_id} with {method} distance {correct_dist}")
